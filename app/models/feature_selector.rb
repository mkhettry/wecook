class FeatureSelector
  attr_accessor :category_counts, :category_counts_for_feature
  ...
  def f_n00(category, feature)
    ...
    out = 0
    @category_counts.category_counts.each do |other_category|
  ...
  def score(category, feature)
    n = Float(@total_lines)
    n11 = f_n11(category, feature)
    n1dot = f_n1dot(category)
    ndot1 = f_ndot1(feature)
    n01 = f_n01(category, feature)
    n0dot = f_n0dot(category)
    n10 = f_n10(category, feature)
    ndot0 = f_ndot0(feature)
    n00 = f_n00(category, feature)

    score = 0.0
    if (n11>0.0 and n01>0 and n10>0.0 and n00>0.0)
      score = (n11/n)*log2(n*n11/(n1dot*ndot1)) + (n01/n)*log2((n*n01)/(n0dot*ndot1)) + (n10/n)*log2(n*n10/(n1dot*ndot0)) + (n00/n)*log2(n*n00/(n0dot*ndot0))
    end
    score
  end
